<?php
/**
 * Script to delete UV test reports submitted before today
 */
...
try {
    ...
    echo "<h2>Deleting Old UV Test Reports</h2>";
    echo "<p>Today's date: <strong>$today</strong></p>";
    
    // First, count the records before today
    $countQuery = "SELECT COUNT(*) as total FROM weathering_exposure_reports WHERE DATE(created_at) < '$today'";
    ...
    if ($totalRecords > 0) {
        ...
        $reportIds = [];
        $idsQuery = "SELECT id FROM weathering_exposure_reports WHERE DATE(created_at) < '$today'";
        ...
        if (!empty($reportIds)) {
            ...
            echo "<p>Deleting reports from weathering_exposure_reports table...</p>";
            $conn->query("DELETE FROM weathering_exposure_reports WHERE DATE(created_at) < '$today'");
    ...
    $remainingQuery = "SELECT COUNT(*) as total FROM weathering_exposure_reports";
    ...
    echo "<br><a href='forms/weathering_exposure_test.php'>← Back to UV Test Form</a>";
