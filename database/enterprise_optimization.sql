...
-- Adds indexes, partitioning, and performance enhancements
...
ALTER TABLE qc_test_orders ADD FULLTEXT INDEX idx_search_test_data(test_data) IF NOT EXISTS;

-- 3. Optimize table structures
OPTIMIZE TABLE qc_test_orders;
OPTIMIZE TABLE new_user;
...
-- 5. Add system health check table
...
-- 7. Add API access log table (for future API endpoints)
...
-- 10. Add query performance tracking
...
-- 11. Create stored procedure for cleanup
DELIMITER $$
...
BEGIN
    ...
    -- Clean old system health checks (older than 30 days)
    DELETE FROM system_health_checks WHERE checked_at < DATE_SUB(NOW(), INTERVAL 30 DAY);
    
    -- Clean old performance metrics (older than 60 days)
    DELETE FROM system_performance WHERE recorded_at < DATE_SUB(NOW(), INTERVAL 60 DAY);
    
    -- Clean old error logs (older than 90 days)
    DELETE FROM error_log WHERE occurred_at < DATE_SUB(NOW(), INTERVAL 90 DAY);
    
    SELECT 'Cleanup completed' AS status;
END$$
DELIMITER ;

-- 12. Create views for common queries (performance optimization)
CREATE OR REPLACE VIEW v_pending_checker_reports AS
SELECT 
    qto.id,
    qto.report_number,
    qto.status,
...
SELECT 
    ...
    u.last_activity
FROM new_user u
LEFT JOIN audit_log al ON u.id = al.user_id AND al.created_at > DATE_SUB(NOW(), INTERVAL 30 DAY)
WHERE u.is_active = 1
...
SELECT 'Enterprise optimization completed successfully!' AS message;
