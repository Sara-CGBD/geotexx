...
$query = "SELECT 
    ...
    p.project_name,
    COALESCE(
        (SELECT full_name FROM new_user WHERE id = c.reporter_id LIMIT 1),
        (SELECT username FROM new_user WHERE id = c.reporter_id LIMIT 1),
        (SELECT username FROM users WHERE id = c.reporter_id LIMIT 1),
        'Admin'
    ) as reporter_name
FROM cnc_entries c
...
$totalRolls = count($entries);
$totalCuttingQty = array_sum(array_column($entries, 'cutting_roll_quantity'));
...
$hourlyQuery = "SELECT 
    ...
    HOUR(c.date_time) as production_hour,
    COUNT(*) as entry_count,
    SUM(c.cutting_roll_quantity) as total_qty,
    CASE 
        WHEN HOUR(c.date_time) >= 8 AND HOUR(c.date_time) < 20 THEN 'Day'
        ELSE 'Night'
...
if ($dateFrom) {
    $hourlyQuery .= " AND c.date_time >= '$dateFrom 00:00:00'";
}
if ($dateTo) {
    $hourlyQuery .= " AND c.date_time <= '$dateTo 23:59:59'";
}
...
<head>
    ...
    <title>CNC Cutting Summary</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        table { width: 100%; border-collapse: collapse; font-size: 0.85em; min-width: 1200px; }
        ...
        table th:nth-child(2), table td:nth-child(2) { min-width: 120px; } /* Entry ID */
        ...
        table th:nth-child(5), table td:nth-child(5) { min-width: 150px; } /* Reference Number */
        table th:nth-child(6), table td:nth-child(6) { min-width: 140px; } /* CNC Cutting Batch */
        table th:nth-child(7), table td:nth-child(7) { min-width: 100px; } /* CNC Machine */
        table th:nth-child(8), table td:nth-child(8) { min-width: 120px; } /* Project */
        table th:nth-child(9), table td:nth-child(9) { min-width: 100px; } /* Cutting Roll Qty */
        table th:nth-child(10), table td:nth-child(10) { min-width: 120px; } /* Bag Size */
...
<body>
<div class="container">
    <h1><i class="fas fa-cut"></i> CNC Cutting Summary Report</h1>
    <p class="subtitle">Number of rolls cut by types and bag sizes</p>
    
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-value"><?php echo number_format($totalRolls); ?></div>
            <div class="stat-label">Total CNC Entries</div>
        </div>
        <div class="stat-card green">
            <div class="stat-value"><?php echo number_format($totalCuttingQty); ?></div>
            <div class="stat-label">Total Rolls Cut</div>
        </div>
    </div>
    ...
    <form method="GET" action="">
        <div class="filters">
            <div class="filter-row">
                ...
                <div class="filter-group">
                    <label><i class="fas fa-box"></i> Bag Size</label>
                    <select name="bag_size">
                        <option value="">All Sizes</option>
                        <?php foreach ($bagSizeList as $size): ?>
                ...
                <div class="filter-group">
                    ...
                    <a href="cnc_cutting_summary.php" class="filter-btn reset-btn" style="text-decoration: none; display: inline-block; text-align: center; line-height: 2;">Reset</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <h3 class="section-title">Detailed CNC Entries</h3>
        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td><strong><?php echo htmlspecialchars($entry['cnc_id'] ?? 'N/A'); ?></strong></td>
                    ...
                    <td><?php echo htmlspecialchars($entry['cnc_machine_id'] ?? 'N/A'); ?></td>
                    <td><?php echo htmlspecialchars($entry['project_name'] ?? 'N/A'); ?></td>
                    <td><?php echo number_format($entry['cutting_roll_quantity'] ?? 0); ?></td>
                    <td><?php echo htmlspecialchars($entry['bag_size']); ?></td>
    ...
    <h3 class="section-title">Hourly CNC Production</h3>
...
function exportToCSV() {
    ...
    link.setAttribute('download', 'cnc_cutting_summary_' + new Date().toISOString().slice(0,10) + '.csv');
