...
\t\t\t\t\t\t\t\t\t\t</div>
        ...
        function displayDetailedOpeningSizeResults(suffixId, targetPercent, result) {
            ...
            detailsHTML += `<strong>O${targetPercent} = ${result.result} mm (${(result.result * 1000).toFixed(0)} �m)</strong>`;
            
            // Create final result display
            const finalResultHTML = `Apparent Opening Size(O${targetPercent}): ${result.result} mm (${(result.result * 1000).toFixed(0)} �m)`;
