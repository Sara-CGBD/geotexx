...
// Fetch store received entries for reference dropdown (exclude only if tenacity fiber test already done)
$storeEntries = [];
try {
    ...
    error_log("Error fetching store entries for tenacity fiber test: " . $e->getMessage());
}
...
// Enhanced backend functionality for Tenacity of Fiber Report
class TenacityFiberReportHandler {
    private $conn;
    private $reporter_id;
    ...
    public function getNextReportNumber() {
        ...
        try {
            ...
            $pattern = 'TFR-' . $dayKey . '-%';
            $countStmt = $this->conn->prepare("SELECT COUNT(*) as report_count FROM tenacity_fiber_reports WHERE report_number LIKE ?");
            ...
            $countStmt->close();
            return "TFR-{$dayKey}-" . str_pad($counter, 5, '0', STR_PAD_LEFT);
            
        } catch (Exception $e) {
            error_log("Tenacity Yarn Report - Get next number error: " . $e->getMessage());
            return "TFR-" . date('YmdHis') . "-" . rand(1000, 9999);
        }
    }
    ...
    public function validateFormData($data) {
        ...
        // Date validation: Test Start Date cannot be earlier than Received Date (same date is allowed)
        if (!empty($data['received_date']) && !empty($data['test_start_date'])) {
            // Extract just the date part (handle datetime-local format)
            $received_str = $data['received_date'];
            if (strpos($received_str, 'T') !== false) {
                $received_str = explode('T', $received_str)[0];
            }
            $received_date = new DateTime($received_str);
            $test_start_date = new DateTime($data['test_start_date']);
            
            // Compare only dates (set time to 00:00:00 for both)
            $received_date->setTime(0, 0, 0);
            $test_start_date->setTime(0, 0, 0);
            
            if ($test_start_date < $received_date) {
                $errors[] = "Test Start Date cannot be earlier than Received Date.";
            }
        }
    ...
    public function saveReport($data) {
        try {
            ...
            $createTable = "CREATE TABLE IF NOT EXISTS tenacity_fiber_reports (
            ...
            $this->conn->query("ALTER TABLE tenacity_fiber_reports ADD COLUMN IF NOT EXISTS store_entry_reference VARCHAR(100) NULL AFTER report_number");
            @$this->conn->query("ALTER TABLE tenacity_fiber_reports MODIFY approved_by VARCHAR(100) NULL");
            @$this->conn->query("ALTER TABLE tenacity_fiber_reports ADD COLUMN IF NOT EXISTS lc_no VARCHAR(100) NULL AFTER sample_description");
            @$this->conn->query("ALTER TABLE tenacity_fiber_reports ADD COLUMN IF NOT EXISTS manufacturer_name VARCHAR(255) NULL AFTER sample_collected_from");
            @$this->conn->query("ALTER TABLE tenacity_fiber_reports MODIFY reference VARCHAR(255) NULL");

            // Check for duplicate report number
            $duplicateCheck = $this->conn->prepare("SELECT id FROM tenacity_fiber_reports WHERE report_number = ?");
            ...
            // Prepare test results JSON - ensure Cross Section always has "Round"
            ...
            if (isset($data['test_results']) && is_array($data['test_results'])) {
                ...
                foreach ($data['test_results'] as $sl_no => $result) {
                    ...
                    // Ensure Cross Section (sl_no 5) always has "Round" as result
                    if ($sl_no == 5) {
                        $test_result = 'Round';
                    }
                    $test_results_processed[] = [
            ...
            $stmt = $this->conn->prepare(
                "INSERT INTO tenacity_fiber_reports (
                    report_number, store_entry_reference, sample_description, lc_no, sample_received_from, sample_collected_from, manufacturer_name, reference,
            ...
            $stmt->bind_param("ssssssssssssddsssiss", 
                $report_number, $data['store_entry_reference'], $sample_description, $lc_no, $sample_received_from, $sample_collected_from, $manufacturer_name, $reference,
                $received_date, $test_start_date, $test_end_date, $others_information,
                $test_temperature, $rh_percent, $test_performed_by, $approved_by,
            ...
            return [
                'success' => true,
                'message' => "Tenacity of Fiber Report saved successfully! Report Number: $report_number",
                'report_id' => $report_id,
    ...
    private function ensureRemarksColumn() {
        $check = $this->conn->query("SHOW COLUMNS FROM tenacity_fiber_reports LIKE 'remarks'");
        if ($check && $check->num_rows == 0) {
            $this->conn->query("ALTER TABLE tenacity_fiber_reports ADD COLUMN remarks TEXT NULL AFTER approved_by");
        }
    }
    ...
    public function cleanupDenierEntries() {
        ...
        try {
            ...
            $result = $this->conn->query("SELECT id, test_results FROM tenacity_fiber_reports WHERE test_results IS NOT NULL AND test_results != ''");
            if ($result) {
                $updated_count = 0;
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($needs_update) {
                        ...
                        $updateStmt = $this->conn->prepare("UPDATE tenacity_fiber_reports SET test_results = ? WHERE id = ?");
                ...
                if ($updated_count > 0) {
                    error_log("Tenacity Fiber Report: Cleaned up Denier entries from $updated_count records");
                }
            }
        } catch (Exception $e) {
    ...
    public function ensureTableExists() {
        ...
        @$this->conn->query("ALTER TABLE tenacity_fiber_reports ADD COLUMN IF NOT EXISTS manufacturer_name VARCHAR(255) NULL AFTER sample_collected_from");
        @$this->conn->query("ALTER TABLE tenacity_fiber_reports MODIFY reference VARCHAR(255) NULL");
        @$this->conn->query("ALTER TABLE tenacity_fiber_reports ADD COLUMN IF NOT EXISTS remarks TEXT NULL AFTER approved_by");
    }
    ...
    public function getPendingReports($limit = 20) {
        ...
        $stmt = $this->conn->prepare(
            "SELECT id, report_number, sample_received_from AS client, sample_description AS material, test_performed_by AS tested_by, status, created_at, updated_at
             FROM tenacity_fiber_reports WHERE status = 'pending' ORDER BY updated_at DESC LIMIT ?"
        );
    ...
    public function approveOrRejectByReportNumber($report_number, $action, $comments = '') {
        ...
        $stmt = $this->conn->prepare("UPDATE tenacity_fiber_reports SET status = ?, approved_by = ?, remarks = ?, updated_at = CURRENT_TIMESTAMP WHERE report_number = ?");
    ...
    public function getReportStats() {
        ...
        try {
            $result = $this->conn->query("SELECT status, COUNT(*) as count FROM tenacity_fiber_reports GROUP BY status");
            if ($result) {
                while ($row = $result->fetch_assoc()) {
...
$TenacityFiberHandler = new TenacityFiberReportHandler($conn, $reporter_id, $reporter_name, $reporter_full_name);
$TenacityFiberHandler->ensureTableExists();
// Clean up any Denier entries from existing records (run every time to ensure cleanup)
$TenacityFiberHandler->cleanupDenierEntries();

// Additional direct cleanup - remove any Denier entries immediately
try {
    $cleanup_result = $conn->query("SELECT id, test_results FROM tenacity_fiber_reports WHERE test_results IS NOT NULL AND test_results != '' AND test_results LIKE '%denier%'");
    if ($cleanup_result && $cleanup_result->num_rows > 0) {
        while ($row = $cleanup_result->fetch_assoc()) {
            $test_results = json_decode($row['test_results'], true);
            if (is_array($test_results)) {
                $cleaned = array_filter($test_results, function($item) {
                    $param = '';
                    if (isset($item['parameter'])) $param = strtolower(trim($item['parameter']));
                    elseif (isset($item['param'])) $param = strtolower(trim($item['param']));
                    return $param !== 'denier';
                });
                $cleaned_json = json_encode(array_values($cleaned), JSON_UNESCAPED_UNICODE);
                $updateStmt = $conn->prepare("UPDATE tenacity_fiber_reports SET test_results = ? WHERE id = ?");
                $updateStmt->bind_param("si", $cleaned_json, $row['id']);
                $updateStmt->execute();
                $updateStmt->close();
            }
    ...
    error_log("Direct Denier cleanup error: " . $e->getMessage());
}

// Fetch pending reports (hide from AGM view)
$pending_reports = [];
$is_agm = in_array($user_role, ['agm ops', 'agm operations']);
$showPendingQueue = $TenacityFiberHandler->canApproveReports() && !$is_agm;
if ($showPendingQueue) {
    $pending_reports = $TenacityFiberHandler->getPendingReports(20);
}
...
if (isset($_GET['view']) && !empty($_GET['view'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM tenacity_fiber_reports WHERE id = ?");
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM tenacity_fiber_reports WHERE id = ? AND reporter_id = ?");
...
if (!$editMode && !$viewMode) {
    $stmt = $conn->prepare("SELECT * FROM tenacity_fiber_reports WHERE reporter_id = ? ORDER BY created_at DESC LIMIT 1");
...
if (!$editMode && !$viewMode) {
    $generated_report_number = $TenacityFiberHandler->getNextReportNumber();
    ...
    // Ensure table exists before querying
    $TenacityFiberHandler->ensureTableExists();
    $pattern_check = 'TFR-' . $dayKey_check . '-%';
    $fresh_count = $conn->query("SELECT COUNT(*) as cnt FROM tenacity_fiber_reports WHERE report_number LIKE '{$pattern_check}'");
    if ($fresh_count && $row_check = $fresh_count->fetch_assoc()) {
        $next_num = (int)$row_check['cnt'] + 1;
        $generated_report_number = "TFR-{$dayKey_check}-" . str_pad($next_num, 5, '0', STR_PAD_LEFT);
    }
}

// Test parameters with their standards and units (define early for use in form submission)
$test_parameters = [
    1 => ['param' => 'Tenacity at Break', 'standards' => ['EN ISO 5079'], 'unit' => 'cN/dTex'],
    2 => ['param' => 'Std Deviation', 'standards' => ['EN ISO 5079'], 'unit' => 'cN/dTex'],
    3 => ['param' => 'CV%', 'standards' => ['EN ISO 5079'], 'unit' => '%'],
    4 => ['param' => 'Elongation at Break', 'standards' => ['EN ISO 5079'], 'unit' => '%'],
    5 => ['param' => 'Cross Section', 'standards' => ['-'], 'unit' => '-', 'fixed_value' => 'Round']
];
    ...
    if ($editMode && isset($_POST['report_id'])) {
        ...
        if (isset($_POST['test_results']) && is_array($_POST['test_results'])) {
            foreach ($_POST['test_results'] as $sl_no => $result) {
                ...
                // Ensure Cross Section always has "Round" as result
                $test_result = $result['result'] ?? '';
                if ($sl_no == 5 && isset($test_parameters[5]['fixed_value'])) {
                    $test_result = $test_parameters[5]['fixed_value'];
                }
                $updated_results[] = [
                    'sl_no' => $sl_no,
                    'parameter' => $param_name,
        ...
        $updateStmt = $conn->prepare("
            UPDATE tenacity_fiber_reports 
            SET store_entry_reference = ?, sample_description = ?, lc_no = ?, sample_received_from = ?, sample_collected_from = ?, manufacturer_name = ?,
        ...
        if ($updateStmt->execute()) {
            if (in_array($user_role, ['admin', 'agm ops', 'agm operations'])) {
                $message = "Tenacity of Fiber Report updated and auto-approved! Report Number: " . $editData['report_number'];
            } else {
                $message = "Tenacity of Fiber Report updated and resubmitted! Report Number: " . $editData['report_number'] . " - Status: Pending Approval";
            }
            $_SESSION['success_message'] = $message;
            header("Location: tenacity_fiber_report.php?success=1&t=" . time());
            exit();
        } else {
        ...
        // Create new report
        $actual_report_number = $TenacityFiberHandler->generateAndIncrementReportNumber();
        $_POST['report_number'] = $actual_report_number;
        
        $result = $TenacityFiberHandler->saveReport($_POST);
        if ($result['success']) {
            if (in_array($user_role, ['admin', 'agm ops', 'agm operations'])) {
                $message = "Tenacity of Fiber Report saved and auto-approved! Report Number: " . $actual_report_number;
            } else {
                $message = "Tenacity of Fiber Report submitted successfully! Report Number: " . $actual_report_number . " - Status: Pending Approval";
            }
            
            $_SESSION['success_message'] = $message;
            $success_msg = urlencode($message);
            header("Location: tenacity_fiber_report.php?success=1&msg={$success_msg}&t=" . time());
            exit();
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wf_action'])) {
    ...
    if ($wf_report_number === '') {
        ...
        $result = $TenacityFiberHandler->approveOrRejectByReportNumber($wf_report_number, $wf_action, $wf_comment);
        if ($result['success']) {
...
$report_stats = $TenacityFiberHandler->getReportStats();

// Handle AJAX request for previous report information
if (isset($_GET['action']) && $_GET['action'] === 'get_previous_info' && isset($_GET['store_entry'])) {
    header('Content-Type: application/json');
    $storeEntryRef = trim($_GET['store_entry']);
    
    try {
        $TenacityFiberHandler->ensureTableExists();
        $stmt = $conn->prepare("
            SELECT sample_description, lc_no, reference, manufacturer_name, 
                   sample_received_from, sample_collected_from, report_number, created_at
            FROM tenacity_fiber_reports 
            WHERE store_entry_reference = ? 
            ORDER BY created_at DESC 
            LIMIT 1
        ");
        $stmt->bind_param("s", $storeEntryRef);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            echo json_encode(['success' => false, 'message' => 'No previous report found']);
        }
        $stmt->close();
    } catch (Exception $e) {
...
<head>
<meta charset="UTF-8">
<title>Tenacity of Fiber Report (EN ISO 5079)</title>
...
<body>
<div class="container">
  <h1><?php echo $viewMode ? 'View ' : ($editMode ? 'Edit Rejected ' : ''); ?>Tenacity of Fiber Report (EN ISO 5079)</h1>
  ...
  <table style="width:100%; border-collapse:collapse; margin-top:20px;">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td style="border:1px solid #ddd; padding:10px; text-align:left;">EN ISO 5079</td>
  ...
  <div id="fiberTenacityRejectModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); overflow-y:auto;">
      <div class="modal-content" style="background:#fff; margin:2% auto; padding:20px; border-radius:12px; width:90%; max-width:500px; max-height:90vh; display:flex; flex-direction:column; box-shadow:0 4px 20px rgba(0,0,0,0.3);">
          <span class="close" onclick="closeFiberTenacityRejectModal()" style="float:right; font-size:24px; font-weight:bold; cursor:pointer; color:#aaa; line-height:1;">&times;</span>
          <div class="modal-header" style="font-size:18px; font-weight:600; margin-bottom:12px; color:#e74c3c; flex-shrink:0;">
              <i class="fas fa-exclamation-triangle"></i> Reject Tenacity of Fiber Test
          </div>
          
          <form method="POST" id="fiberTenacityRejectForm" onsubmit="return submitFiberTenacityRejection(event)">
              ...
              <div class="modal-body" style="flex:1; overflow-y:auto; padding-right:5px;">
                  <div class="checkbox-group" style="margin:10px 0; max-height:200px; overflow-y:auto; padding:5px; border:1px solid #e0e0e0; border-radius:6px;">
                      ...
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Sample contamination" id="fiber_tenacity_reason1" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason1" style="cursor:pointer; flex:1; font-size:12px;">Sample contamination</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Incorrect test procedure (EN ISO 5079)" id="fiber_tenacity_reason2" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason2" style="cursor:pointer; flex:1; font-size:12px;">Incorrect test procedure (EN ISO 5079)</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Out of specification results" id="fiber_tenacity_reason3" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason3" style="cursor:pointer; flex:1; font-size:12px;">Out of specification results</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Incomplete test data" id="fiber_tenacity_reason4" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason4" style="cursor:pointer; flex:1; font-size:12px;">Incomplete test data</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Equipment calibration issue" id="fiber_tenacity_reason5" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason5" style="cursor:pointer; flex:1; font-size:12px;">Equipment calibration issue</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Documentation error" id="fiber_tenacity_reason6" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason6" style="cursor:pointer; flex:1; font-size:12px;">Documentation error</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="fiber_tenacity_rejection_reasons[]" value="Fiber quality issue" id="fiber_tenacity_reason7" style="transform:scale(1.1); cursor:pointer;">
                          <label for="fiber_tenacity_reason7" style="cursor:pointer; flex:1; font-size:12px;">Fiber quality issue</label>
                      </div>
                  </div>
                  
                  <textarea name="fiber_tenacity_remarks" class="remarks-box" placeholder="Additional comments (optional)" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:6px; margin-top:10px; min-height:80px;"></textarea>
              </div>
  ...
  <script>
  function submitFiberTenacityRejection(e) {
      e.preventDefault();
      ...
      actionInput.name = 'fiber_tenacity_action';
      actionInput.value = 'rejected';
      submitForm.appendChild(actionInput);
      
      const reportInput = document.createElement('input');
      reportInput.type = 'hidden';
      reportInput.name = 'fiber_tenacity_report_number';
      reportInput.value = document.getElementById('fiber_tenacity_report_number_input').value;
      ...
      checkboxes.forEach(function(checkbox) {
          ...
          reasonInput.name = 'fiber_tenacity_rejection_reasons[]';
          reasonInput.value = checkbox.value;
          submitForm.appendChild(reasonInput);
      });
      
      // Add remarks if any
      const remarksField = form.querySelector('textarea[name="fiber_tenacity_remarks"]');
      if (remarksField && remarksField.value.trim()) {
          const remarksInput = document.createElement('input');
          remarksInput.type = 'hidden';
          remarksInput.name = 'fiber_tenacity_remarks';
  ...
  <?php if (!$TenacityFiberHandler->canApproveReports() && !$editMode): ?>
  <!-- Rejected Reports for Tester to Review - Only show when NOT in edit mode -->
  <?php $rejected = $TenacityFiberHandler->getRejectedReportsForUser($reporter_id); if (!empty($rejected)): ?>
    ...
    <table class="test-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <a href="tenacity_fiber_report.php?id=<?php echo $rj['id']; ?>" class="submit-btn" style="padding:6px 10px; text-decoration:none; display:inline-block; background:#f39c12; color:#fff;">Edit & Resubmit</a>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
  ...
  <form method="POST" action="" onsubmit="return validateTestDate();">
    <?php if ($editMode): ?>
    ...
    <!-- Previously Filled Information Section -->
    <div id="previousInfoSection" style="display: none; background: #e8f4f8; border: 1px solid #3498db; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
      <h3 style="margin: 0 0 15px 0; color: #2980b9; font-size: 16px; display: flex; align-items: center;">
        <i class="fas fa-info-circle" style="margin-right: 8px;"></i> Previously Filled Information
      </h3>
      <div class="form-row" style="margin-bottom: 10px;">
        <div class="form-group" style="flex: 1;">
          <label style="font-size: 12px; color: #7f8c8d;">Sample Description:</label>
          <div id="prev_sample_description" style="padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px; color: #2c3e50; font-size: 14px;">-</div>
        </div>
        <div class="form-group" style="flex: 1;">
          <label style="font-size: 12px; color: #7f8c8d;">LC No:</label>
          <div id="prev_lc_no" style="padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px; color: #2c3e50; font-size: 14px;">-</div>
        </div>
      </div>
      <div class="form-row" style="margin-bottom: 10px;">
        <div class="form-group" style="flex: 1;">
          <label style="font-size: 12px; color: #7f8c8d;">Ref:</label>
          <div id="prev_reference" style="padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px; color: #2c3e50; font-size: 14px;">-</div>
        </div>
        <div class="form-group" style="flex: 1;">
          <label style="font-size: 12px; color: #7f8c8d;">Manufacturer Name:</label>
          <div id="prev_manufacturer_name" style="padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px; color: #2c3e50; font-size: 14px;">-</div>
        </div>
      </div>
      <div class="form-row" style="margin-bottom: 10px;">
        <div class="form-group" style="flex: 1;">
          <label style="font-size: 12px; color: #7f8c8d;">Sample Received From:</label>
          <div id="prev_sample_received_from" style="padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px; color: #2c3e50; font-size: 14px;">-</div>
        </div>
        <div class="form-group" style="flex: 1;">
          <label style="font-size: 12px; color: #7f8c8d;">Sample Collected From:</label>
          <div id="prev_sample_collected_from" style="padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px; color: #2c3e50; font-size: 14px;">-</div>
        </div>
      </div>
      <div style="margin-top: 10px; font-size: 12px; color: #7f8c8d; font-style: italic;">
        <i class="fas fa-lightbulb"></i> This information is from the most recent report for this store entry. You can use it as reference or modify as needed.
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <input type="text" name="sample_description" id="sample_description" value="<?php 
          if ($editMode) {
      ...
      <div class="form-group">
        <label>LC No:</label>
        <input type="text" name="lc_no" id="lc_no" value="<?php 
          if ($editMode) {
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <input type="datetime-local" name="received_date" id="received_date" value="<?php 
        ...
        ?>" required onchange="validateTestDate()">
      </div>
      <div class="form-group">
        ...
        ?>" required onchange="validateTestDate()">
        <small id="date_error" style="color: #e74c3c; display: none;">Test Start Date cannot be earlier than Received Date.</small>
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <input type="date" name="test_end_date" value="<?php 
          if ($editMode) {
    ...
    <table class="test-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <?php if (isset($param['fixed_value'])): ?>
              <input type="hidden" name="test_results[<?php echo $sl_no; ?>][unit]" value="<?php echo htmlspecialchars($param['unit']); ?>">
              <span style="font-weight: bold; color: #7f8c8d;"><?php echo htmlspecialchars($param['unit']); ?></span>
            <?php else: ?>
          ...
          <td>
            <?php if (isset($param['fixed_value'])): ?>
              <input type="hidden" name="test_results[<?php echo $sl_no; ?>][result]" value="<?php echo htmlspecialchars($param['fixed_value']); ?>">
              <span style="font-weight: bold; background: #f8f9fa; padding: 5px 10px; border-radius: 4px; display: inline-block;"><?php echo htmlspecialchars($param['fixed_value']); ?></span>
            <?php else: ?>
...
// Validate that Test Start Date is not earlier than Received Date
function validateTestDate() {
    const receivedDateInput = document.getElementById('received_date');
    const testDateInput = document.getElementById('test_start_date');
    const errorMsg = document.getElementById('date_error');
    
    if (receivedDateInput && testDateInput && receivedDateInput.value && testDateInput.value) {
        // Handle datetime-local format (YYYY-MM-DDTHH:mm) - extract just the date part
        const receivedValue = receivedDateInput.value.split('T')[0];
        const received = new Date(receivedValue + 'T00:00:00');
        const tested = new Date(testDateInput.value + 'T00:00:00');
        
        // Compare dates only (same date is allowed)
        if (tested < received) {
            errorMsg.style.display = 'block';
            testDateInput.setCustomValidity('Test Start Date cannot be earlier than Received Date.');
            return false;
        } else {
            errorMsg.style.display = 'none';
            testDateInput.setCustomValidity('');
            return true;
        }
    }
    return true;
...
// Auto-select manufacturer name and fetch previous info when store entry reference is selected
document.addEventListener('DOMContentLoaded', function() {
    ...
    const previousInfoSection = document.getElementById('previousInfoSection');
    ...
    if (storeEntrySelect && manufacturerSelect) {
        ...
        // Function to fetch and display previous report information
        function fetchPreviousInfo(storeEntryRef) {
            if (!storeEntryRef || storeEntryRef.trim() === '') {
                if (previousInfoSection) {
                    previousInfoSection.style.display = 'none';
                }
                return;
            }
            
            // Fetch previous report data via AJAX
            fetch('tenacity_fiber_report.php?action=get_previous_info&store_entry=' + encodeURIComponent(storeEntryRef))
                .then(response => response.json())
                .then(data => {
                    if (data.success && data.report) {
                        // Display previous information
                        document.getElementById('prev_sample_description').textContent = data.report.sample_description || '-';
                        document.getElementById('prev_lc_no').textContent = data.report.lc_no || '-';
                        document.getElementById('prev_reference').textContent = data.report.reference || '-';
                        document.getElementById('prev_manufacturer_name').textContent = data.report.manufacturer_name || '-';
                        document.getElementById('prev_sample_received_from').textContent = data.report.sample_received_from || '-';
                        document.getElementById('prev_sample_collected_from').textContent = data.report.sample_collected_from || '-';
                        
                        if (previousInfoSection) {
                            previousInfoSection.style.display = 'block';
                        }
                    } else {
                        if (previousInfoSection) {
                            previousInfoSection.style.display = 'none';
                ...
                .catch(error => {
                    console.error('Error fetching previous info:', error);
                    if (previousInfoSection) {
                        previousInfoSection.style.display = 'none';
                    }
                });
        ...
        // Handle store entry selection change
        function handleStoreEntryChange() {
            const selectedValue = storeEntrySelect.value;
            autoSelectManufacturer();
            fetchPreviousInfo(selectedValue);
        }
        
        // Auto-select on change
        storeEntrySelect.addEventListener('change', handleStoreEntryChange);
        
        // Auto-select on page load if store entry is already selected (edit mode)
        if (storeEntrySelect.selectedIndex > 0) {
            handleStoreEntryChange();
        }
    }
});
</script>
