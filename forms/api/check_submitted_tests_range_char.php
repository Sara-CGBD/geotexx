<?php
/**
 * API to check which references in a range have been submitted for Characteristics Test
 */
session_start();
...
// Get all submitted characteristics tests
$submitted_refs = [];
$stmt = $conn->prepare("
    SELECT DISTINCT reference_number, bundle_reference, status, created_at
    FROM characteristics_tests
    WHERE status IN ('pending', 'checked', 'approved')
        ...
        if (!empty($bundleRef) && strpos($bundleRef, '|') !== false) {
            ...
            if (count($parts) === 2) {
                ...
                if ($bundleFrom === $from_reference && $bundleTo === $to_reference) {
                    // This entire range has been submitted
                    $submitted_refs[] = [
                        'reference' => $bundleRef,
                        'status' => $row['status'],
                        'submitted_at' => $row['created_at'],
