    ...
    <div class="form-row">
      <div class="form-group" style="grid-column: 1 / -1;">
        ...
        <div id="sun_bulk_reference_selection" style="display:none; margin-bottom:10px; padding:10px; background:#f8f9fa; border:1px solid #ddd; border-radius:6px;">
          ...
          <div id="sun_reference_test_status_container" style="display:none; margin-top:15px; padding:0; background:#ffffff; border:1px solid #e0e0e0; border-radius:8px; box-shadow:0 1px 4px rgba(0,0,0,0.08); max-width:550px; margin-left:auto; margin-right:auto;">
            <div style="padding:12px 16px; background:linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius:8px 8px 0 0; color:white;">
              <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                <div style="display:flex; align-items:center; gap:8px;">
                  ...
                  <h3 style="margin:0; font-size:14px; font-weight:600;" id="sun_reference_status_title">Test Status</h3>
                </div>
                <div style="font-size:11px; opacity:0.95; font-weight:500;" id="sun_reference_status_summary"></div>
              </div>
            </div>
            <div id="sun_reference_test_status_list" style="padding:12px; max-height:300px; overflow-y:auto;">
...
function loadReferencesList() {
    fetch('api/get_sun_references_list.php')
    ...
    Array.from(productRefSelect.options).forEach(option => {
        if (option.value && option.value !== '') {
            ...
            if (optionLine === line) {
                ...
                // If this is a bundle reference, expand it into individual roll references
                if (isBundle && baseRef && rollCount > 1) {
                    // Generate individual roll references (1, 2, 3, ..., rollCount)
                    for (let i = 1; i <= rollCount; i++) {
                        const individualRef = baseRef + '-' + i;
                        lineReferences.push({
                            value: individualRef,
                            text: individualRef,
                            isBundle: false, // Individual roll, not the bundle itself
                            baseRef: baseRef,
                            rollCount: rollCount
                        });
                    }
                } else {
                    // Single reference or individual roll
                    lineReferences.push({
                        value: option.value,
...
function applySunBulkReferenceSelection() {
    ...
    checkSunReferenceTestStatus(fromRef, toRef);
}

// Check which references in range have been submitted for Sun Test
function checkSunReferenceTestStatus(fromRef, toRef) {
    const container = document.getElementById('sun_reference_test_status_container');
    const statusList = document.getElementById('sun_reference_test_status_list');
    const statusTitle = document.getElementById('sun_reference_status_title');
    const statusSummary = document.getElementById('sun_reference_status_summary');
    ...
    fetch(`api/check_submitted_tests_range_sun.php?from_reference=${encodeURIComponent(fromRef)}&to_reference=${encodeURIComponent(toRef)}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                displaySunReferenceStatus(data, fromRef, toRef);
            } else {
...
function displaySunReferenceStatus(data, fromRef, toRef) {
    const statusList = document.getElementById('sun_reference_test_status_list');
    const statusSummary = document.getElementById('sun_reference_status_summary');
    ...
    const fromSelect = document.getElementById('sun_from_reference');
    const toSelect = document.getElementById('sun_to_reference');
    const allRefs = [];
