...
// Fetch store received entries for reference dropdown (exclude only if cut length fiber test already done)
$storeEntries = [];
try {
    ...
    error_log("Error fetching store entries for cut length fiber test: " . $e->getMessage());
}
...
// Enhanced backend functionality for Cut Length of Fiber Report
class CutLengthFiberReportHandler {
    private $conn;
    private $reporter_id;
    ...
    public function getNextReportNumber() {
        ...
        try {
            ...
            $pattern = 'CLF-' . $dayKey . '-%';
            $countStmt = $this->conn->prepare("SELECT COUNT(*) as report_count FROM cut_length_fiber_reports WHERE report_number LIKE ?");
            ...
            $countStmt->close();
            return "CLF-{$dayKey}-" . str_pad($counter, 5, '0', STR_PAD_LEFT);
            
        } catch (Exception $e) {
            error_log("Tenacity Yarn Report - Get next number error: " . $e->getMessage());
            return "CLF-" . date('YmdHis') . "-" . rand(1000, 9999);
        }
    }
    ...
    public function saveReport($data) {
        try {
            ...
            $createTable = "CREATE TABLE IF NOT EXISTS cut_length_fiber_reports (
            ...
            $this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN IF NOT EXISTS store_entry_reference VARCHAR(100) NULL AFTER report_number");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports MODIFY sample_description TEXT NULL");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN IF NOT EXISTS lc_no VARCHAR(100) NULL AFTER sample_description");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports MODIFY received_date DATE NULL");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports MODIFY test_start_date DATE NULL");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports MODIFY test_end_date DATE NULL");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports MODIFY approved_by VARCHAR(100) NULL");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN IF NOT EXISTS manufacturer_name VARCHAR(255) NULL AFTER lc_no");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN IF NOT EXISTS remarks TEXT NULL AFTER approved_by");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN IF NOT EXISTS rejected_by VARCHAR(100) NULL AFTER remarks");
            @$this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN IF NOT EXISTS rejected_at TIMESTAMP NULL AFTER rejected_by");

            // Check for duplicate report number
            $duplicateCheck = $this->conn->prepare("SELECT id FROM cut_length_fiber_reports WHERE report_number = ?");
            ...
            $stmt = $this->conn->prepare(
                "INSERT INTO cut_length_fiber_reports (
            ...
            return [
                'success' => true,
                'message' => "Cut Length of Fiber Report saved successfully! Report Number: $report_number",
                'report_id' => $report_id,
    ...
    private function ensureRemarksColumn() {
        $check = $this->conn->query("SHOW COLUMNS FROM cut_length_fiber_reports LIKE 'remarks'");
        if ($check && $check->num_rows == 0) {
            $this->conn->query("ALTER TABLE cut_length_fiber_reports ADD COLUMN remarks TEXT NULL AFTER approved_by");
        }
    }
    ...
    public function getPendingReports($limit = 20) {
        ...
        $stmt = $this->conn->prepare(
            "SELECT id, report_number, sample_description AS material, test_performed_by AS tested_by, status, created_at, updated_at
             FROM cut_length_fiber_reports WHERE status = 'pending' ORDER BY updated_at DESC LIMIT ?"
        );
    ...
    public function approveOrRejectByReportNumber($report_number, $action, $comments = '') {
        ...
        $stmt = $this->conn->prepare("UPDATE cut_length_fiber_reports SET status = ?, approved_by = ?, remarks = ?, updated_at = CURRENT_TIMESTAMP WHERE report_number = ?");
    ...
    public function getReportStats() {
        ...
        try {
            $result = $this->conn->query("SELECT status, COUNT(*) as count FROM cut_length_fiber_reports GROUP BY status");
            if ($result) {
                while ($row = $result->fetch_assoc()) {
...
$CutLengthFiberHandler = new CutLengthFiberReportHandler($conn, $reporter_id, $reporter_name, $reporter_full_name);

// Fetch pending reports (hide from AGM view per request)
$pending_reports = [];
$is_agm = in_array($user_role, ['agm ops', 'agm operations']);
$showPendingQueue = $CutLengthFiberHandler->canApproveReports() && !$is_agm;
if ($showPendingQueue) {
    $pending_reports = $CutLengthFiberHandler->getPendingReports(20);
}
...
if (isset($_GET['view']) && !empty($_GET['view'])) {
    ...
    $CutLengthFiberHandler->ensureTableExists();
    
    $stmt = $conn->prepare("SELECT * FROM cut_length_fiber_reports WHERE id = ?");
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM cut_length_fiber_reports WHERE id = ? AND reporter_id = ?");
...
if (!$editMode && !$viewMode) {
    $stmt = $conn->prepare("SELECT * FROM cut_length_fiber_reports WHERE reporter_id = ? ORDER BY created_at DESC LIMIT 1");
...
if (!$editMode && !$viewMode) {
    $generated_report_number = $CutLengthFiberHandler->getNextReportNumber();
    ...
    // Ensure table exists before querying
    $CutLengthFiberHandler->ensureTableExists();
    $pattern_check = 'CLF-' . $dayKey_check . '-%';
    $fresh_count = $conn->query("SELECT COUNT(*) as cnt FROM cut_length_fiber_reports WHERE report_number LIKE '{$pattern_check}'");
    if ($fresh_count && $row_check = $fresh_count->fetch_assoc()) {
        $next_num = (int)$row_check['cnt'] + 1;
        $generated_report_number = "CLF-{$dayKey_check}-" . str_pad($next_num, 5, '0', STR_PAD_LEFT);
    }
}

// Test parameters with their standards and units (define early for use in form submission)
$test_parameters = [
    1 => ['param' => 'Cut Length', 'standards' => ['ASTM D5103'], 'unit' => 'mm']
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_report'])) {
    ...
    if ($editMode && isset($_POST['report_id'])) {
        ...
        $updateStmt = $conn->prepare("
            UPDATE cut_length_fiber_reports 
        ...
        if ($updateStmt->execute()) {
            if (in_array($user_role, ['admin', 'agm ops', 'agm operations'])) {
                $message = "Cut Length of Fiber Report updated and auto-approved! Report Number: " . $editData['report_number'];
            } else {
                $message = "Cut Length of Fiber Report updated and resubmitted! Report Number: " . $editData['report_number'] . " - Status: Pending Approval";
            }
            $_SESSION['success_message'] = $message;
            header("Location: cut_length_fiber_report.php?success=1&t=" . time());
            exit();
        } else {
        ...
        // Create new report
        $actual_report_number = $CutLengthFiberHandler->generateAndIncrementReportNumber();
        $_POST['report_number'] = $actual_report_number;
        
        $result = $CutLengthFiberHandler->saveReport($_POST);
        if ($result['success']) {
            if (in_array($user_role, ['admin', 'agm ops', 'agm operations'])) {
                $message = "Cut Length of Fiber Report saved and auto-approved! Report Number: " . $actual_report_number;
            } else {
                $message = "Cut Length of Fiber Report submitted successfully! Report Number: " . $actual_report_number . " - Status: Pending Approval";
            }
            
            $_SESSION['success_message'] = $message;
            $success_msg = urlencode($message);
            header("Location: cut_length_fiber_report.php?success=1&msg={$success_msg}&t=" . time());
            exit();
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wf_action'])) {
    ...
    if ($wf_report_number === '') {
        ...
        $result = $CutLengthFiberHandler->approveOrRejectByReportNumber($wf_report_number, $wf_action, $wf_comment);
        if ($result['success']) {
...
$report_stats = $CutLengthFiberHandler->getReportStats();
...
<head>
<meta charset="UTF-8">
<title>Cut Length of Fiber Report (ASTM D5103)</title>
...
<body>
<div class="container">
  <h1><?php echo $viewMode ? 'View ' : ($editMode ? 'Edit Rejected ' : ''); ?>Cut Length of Fiber Report (ASTM D5103)</h1>
  ...
  <table style="width:100%; border-collapse:collapse; margin-top:20px;">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td style="border:1px solid #ddd; padding:10px; text-align:left;">ASTM D5103</td>
  ...
  <div id="cutLengthFiberRejectModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); overflow-y:auto;">
      <div class="modal-content" style="background:#fff; margin:2% auto; padding:20px; border-radius:12px; width:90%; max-width:500px; max-height:90vh; display:flex; flex-direction:column; box-shadow:0 4px 20px rgba(0,0,0,0.3);">
          <span class="close" onclick="closeCutLengthFiberRejectModal()" style="float:right; font-size:24px; font-weight:bold; cursor:pointer; color:#aaa; line-height:1;">&times;</span>
          <div class="modal-header" style="font-size:18px; font-weight:600; margin-bottom:12px; color:#e74c3c; flex-shrink:0;">
              <i class="fas fa-exclamation-triangle"></i> Reject Cut Length of Fiber Test
          </div>
          
          <form method="POST" id="cutLengthFiberRejectForm" onsubmit="return submitCutLengthFiberRejection(event)">
              ...
              <div class="modal-body" style="flex:1; overflow-y:auto; padding-right:5px;">
                  <div class="checkbox-group" style="margin:10px 0; max-height:200px; overflow-y:auto; padding:5px; border:1px solid #e0e0e0; border-radius:6px;">
                      ...
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Sample contamination" id="cut_length_fiber_reason1" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason1" style="cursor:pointer; flex:1; font-size:12px;">Sample contamination</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Incorrect test procedure (ASTM D5103)" id="cut_length_fiber_reason2" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason2" style="cursor:pointer; flex:1; font-size:12px;">Incorrect test procedure (ASTM D5103)</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Out of specification results" id="cut_length_fiber_reason3" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason3" style="cursor:pointer; flex:1; font-size:12px;">Out of specification results</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Incomplete test data" id="cut_length_fiber_reason4" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason4" style="cursor:pointer; flex:1; font-size:12px;">Incomplete test data</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Equipment calibration issue" id="cut_length_fiber_reason5" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason5" style="cursor:pointer; flex:1; font-size:12px;">Equipment calibration issue</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Documentation error" id="cut_length_fiber_reason6" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason6" style="cursor:pointer; flex:1; font-size:12px;">Documentation error</label>
                      </div>
                      <div class="checkbox-item" style="display:flex; align-items:center; gap:8px; padding:6px; border:1px solid #ddd; border-radius:4px; margin-bottom:4px; background:#f8f9fa;">
                          <input type="checkbox" name="cut_length_fiber_rejection_reasons[]" value="Fiber quality issue" id="cut_length_fiber_reason7" style="transform:scale(1.1); cursor:pointer;">
                          <label for="cut_length_fiber_reason7" style="cursor:pointer; flex:1; font-size:12px;">Fiber quality issue</label>
                      </div>
                  </div>
                  
                  <textarea name="cut_length_fiber_remarks" class="remarks-box" placeholder="Additional comments (optional)" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:6px; margin-top:10px; min-height:80px;"></textarea>
              </div>
              
              <div class="modal-footer" style="flex-shrink:0; margin-top:15px; padding-top:15px; border-top:1px solid #ddd; text-align:right;">
                  <button type="button" onclick="closeCutLengthFiberRejectModal()" style="padding:8px 18px; background:#6c757d; color:#fff; border:none; border-radius:6px; cursor:pointer; margin-right:8px; font-size:13px;">
                      Cancel
                  </button>
  ...
  <script>
  function submitCutLengthFiberRejection(e) {
      e.preventDefault();
      ...
      actionInput.name = 'cut_length_fiber_action';
      actionInput.value = 'rejected';
      submitForm.appendChild(actionInput);
      
      const reportInput = document.createElement('input');
      reportInput.type = 'hidden';
      reportInput.name = 'cut_length_fiber_report_number';
      reportInput.value = document.getElementById('cut_length_fiber_report_number_input').value;
      ...
      checkboxes.forEach(function(checkbox) {
          ...
          reasonInput.name = 'cut_length_fiber_rejection_reasons[]';
          reasonInput.value = checkbox.value;
          submitForm.appendChild(reasonInput);
      });
      
      // Add remarks if any
      const remarksField = form.querySelector('textarea[name="cut_length_fiber_remarks"]');
      if (remarksField && remarksField.value.trim()) {
          const remarksInput = document.createElement('input');
          remarksInput.type = 'hidden';
          remarksInput.name = 'cut_length_fiber_remarks';
      ...
      <table class="test-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?php echo htmlspecialchars($pr['client'] ?? 'N/A'); ?></td>
  ...
  <?php if (!$CutLengthFiberHandler->canApproveReports() && !$editMode): ?>
  <!-- Rejected Reports for Tester to Review - Only show when NOT in edit mode -->
  <?php $rejected = $CutLengthFiberHandler->getRejectedReportsForUser($reporter_id); if (!empty($rejected)): ?>
    ...
    <table class="test-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <a href="cut_length_fiber_report.php?id=<?php echo $rj['id']; ?>" class="submit-btn" style="padding:6px 10px; text-decoration:none; display:inline-block; background:#f39c12; color:#fff;">Edit & Resubmit</a>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
