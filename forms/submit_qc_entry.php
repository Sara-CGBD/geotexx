...
// Add original_amount_kg column if it doesn't exist (required by insert)
$checkOriginal = $conn->query("SHOW COLUMNS FROM store_received_entries LIKE 'original_amount_kg'");
if ($checkOriginal && $checkOriginal->num_rows == 0) {
    $alterResult = $conn->query("ALTER TABLE store_received_entries ADD COLUMN original_amount_kg DECIMAL(10,2) NOT NULL DEFAULT 0 AFTER amount_kg");
    if (!$alterResult) {
        error_log("Failed to add original_amount_kg column: " . $conn->error);
    }
}

// Final presence check after possible alter
$verifyOriginal = $conn->query("SHOW COLUMNS FROM store_received_entries LIKE 'original_amount_kg'");
$hasOriginalAmount = ($verifyOriginal && $verifyOriginal->num_rows > 0);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_entry'])) {
    try {
        ...
        // Insert entry (fallback if original_amount_kg column is missing)
        $stmt = null;
        $executed = false;
        ...
        $stmt->bind_param("sssssddsi", $entry_number, $date_time, $shift, $manufacturer_name, $material_type, $amount_kg, $amount_kg, $reported_by, $reporter_id);
                if ($stmt->execute()) {
                    $executed = true;
                } elseif ($stmt->errno === 1054) { // unknown column
                    $stmt->close();
                    $stmt = null;
                }
            }
        ...
        if (!$executed) {
            ...
            $stmt = $conn->prepare("INSERT INTO store_received_entries 
                (entry_number, date_time, shift, manufacturer_name, material_type, amount_kg, reported_by, reporter_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt) {
                $stmt->bind_param("sssssddi", $entry_number, $date_time, $shift, $manufacturer_name, $material_type, $amount_kg, $reported_by, $reporter_id);
...
<div class="container">
  ...
  <?php if ($error): ?>
    <div style="background:#f8d7da;color:#721c24;padding:12px;border-radius:6px;border:1px solid #f5c6cb;margin-bottom:15px;">
      âŒ <?php echo htmlspecialchars($error); ?>
    </div>
  <?php endif; ?>
