...
function filterReferencesByDestination() {
  ...
  options.forEach(option => {
    ...
    if (isBundle) {
      ...
      if (bundleDataJson) {
        try {
          ...
          bundleData.forEach(refData => {
            if (refData.reference_number && selectedRefs.includes(refData.reference_number.toLowerCase())) {
      ...
      // For individual references, check if it's in the selected list
      isAlreadySelected = selectedRefs.includes(refText);
    }
