...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && in_array($role, ['admin', 'agm ops', 'agm operations'])) {
    ...
    $getBundleStmt = $conn->prepare("SELECT bundle_reference FROM sun_test_reports WHERE id = ?");
    ...
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE sun_test_reports SET status = 'approved', approved_by = ?, approved_at = NOW() WHERE $whereClause");
        if ($types === "s") {
        ...
        $stmt = $conn->prepare("UPDATE sun_test_reports SET status = 'rejected', remarks = ?, approved_by = ? WHERE $whereClause");
        if ($types === "s") {
