...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE water_permeability_tests SET status = 'checked', checked_by = ?, checked_at = NOW() WHERE id = ? AND status = 'pending'");
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Approved</title></head><body><script>
                if (window.opener && !window.opener.closed) {
                    ...
                    window.location.href = "../forms/water_permeability_test.php?t=" + new Date().getTime();
                }
        ...
        $stmt = $conn->prepare("UPDATE water_permeability_tests SET status = 'rejected', remarks = ?, checked_by = ? WHERE id = ? AND status = 'pending'");
...
<div class="container">
  ...
  <div style="margin-top:20px;">
    <a href="../forms/water_permeability_test.php" class="btn btn-back">← Back to Dashboard</a>
