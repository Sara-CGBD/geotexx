...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE water_permeability_tests SET status = 'checked', checked_by = ?, checked_at = NOW() WHERE $whereClause");
        if ($types === "s") {
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Approved</title></head><body><script>
                if (window.opener && !window.opener.closed) {
                    ...
                    try {
                        window.opener.postMessage({ type: "report_processed", action: "approved", report_type: "water_permeability", report_id: ' . $report_id . ' }, window.location.origin);
                    } catch(e) {}
            ...
            </script><p>Approved! Redirecting to Lab Testing Dashboard...</p></body></html>';
            exit();
        }
        $stmt->close();
        ...
        if (isset($_POST['rejection_reasons']) && is_array($_POST['rejection_reasons']) && count($_POST['rejection_reasons']) > 0) {
            ...
            header("Location: check_water_permeability.php?id=" . $report_id);
            exit();
        }
        
        // Use the same where clause logic for rejection
        $stmt = $conn->prepare("UPDATE water_permeability_tests SET status = 'rejected', remarks = ?, checked_by = ? WHERE $whereClause");
        if ($types === "s") {
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Rejected</title></head><body><script>
                if (window.opener && !window.opener.closed) {
                    // Send message to parent window to trigger refresh
                    try {
                        window.opener.postMessage({ type: "report_processed", action: "rejected", report_type: "water_permeability", report_id: ' . $report_id . ' }, window.location.origin);
                    } catch(e) {}
                    // Also refresh parent directly as fallback - redirect to Lab Testing Dashboard
                    window.opener.location.href = "lab_testing_dashboard.php?t=" + new Date().getTime();
                    setTimeout(function() { window.close(); }, 100);
            ...
            </script><p>Rejected! Redirecting to Lab Testing Dashboard...</p></body></html>';
            exit();
        }
        $stmt->close();
    }
...
<head>
  <meta charset="UTF-8">
  <title>Check Water Permeability Test - <?php echo htmlspecialchars($report['report_number']); ?></title>
...
<body>
<div class="container">
  <h1> Check Water Permeability Test Report</h1>
